  ...
  <b-container fluid>
        ...
        <b-row>
          <b-col sm="5">
            <div class="form-group">
              ...
              <input
                type="int"
                v-model="user_data.customer_type_id"
                v-validate="{ required: true, min: 6 }"
                name="type"
                class="form-control"
    ...
    <div class="row">
      <template v-for="variant in ['success']">
        <div class="col-md-4 pb-2" v-for="size in ['']" :key="`${variant}_${size}`">
          <b-button
            :size="size"
            v-on:click="onSubmit(user_data)"
            :variant="variant"
          >{{variant}} {{size}}</b-button>
        </div>
      </template>
    </div>
  </b-container>
...
export default {
  ...
  methods: {
    async onSubmit(data) {
      await axios.put("http://localhost:3000/api/customers/"+data.id, data);
